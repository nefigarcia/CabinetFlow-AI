// Cabinet material profiles — org-scoped CRUD list + create.
...
export async function GET(req: NextRequest): Promise<Response> {
  ...
  const rows = await prisma.cabinetMaterialProfile.findMany({
...
export async function POST(req: NextRequest): Promise<Response> {
  ...
  const parsed = cabinetMaterialProfileWriteSchema.safeParse(body);
  if (!parsed.success) {
    return apiError(
  ...
  const created = await prisma.cabinetMaterialProfile.create({
