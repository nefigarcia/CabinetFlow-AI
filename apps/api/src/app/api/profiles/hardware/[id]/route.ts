// Hardware profile — org-scoped GET / PATCH / DELETE.
...
async function findOwned(id: string, orgId: string) {
  return prisma.hardwareProfile.findFirst({ where: { id, orgId } });
}
...
export async function PATCH(req: NextRequest, { params }: Params): Promise<Response> {
  ...
  const parsed = hardwareProfileWriteSchema.partial().safeParse(body);
  if (!parsed.success) {
    return apiError(
  ...
  const updated = await prisma.hardwareProfile.update({
    where: { id: params.id },
    data,
  });
  return ok(updated);
...
export async function DELETE(req: NextRequest, { params }: Params): Promise<Response> {
  ...
  await prisma.hardwareProfile.delete({ where: { id: params.id } });
