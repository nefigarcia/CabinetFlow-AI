// Construction profile — org-scoped GET / PATCH / DELETE.
...
async function findOwned(id: string, orgId: string) {
  return prisma.constructionProfile.findFirst({ where: { id, orgId } });
}
...
export async function PATCH(req: NextRequest, { params }: Params): Promise<Response> {
  ...
  const parsed = constructionProfileWriteSchema.partial().safeParse(body);
  if (!parsed.success) {
    return apiError(
  ...
  const updated = await prisma.constructionProfile.update({
    where: { id: params.id },
    data,
  });
  return ok(updated);
...
export async function DELETE(req: NextRequest, { params }: Params): Promise<Response> {
  ...
  await prisma.constructionProfile.delete({ where: { id: params.id } });
